...
position pawn::deadpos(piece *board[12][8])
{
    ...
    if(colour==1)
        ...
        result.x=1;
    int i;
    for (i=0;board[result.x][i]!=NULL;i++) {
...
bool pawn::valid_move(piece *board[12][8],position final){
    if (colour==1) {
        ...
        if(final.x==mine.x+1&&final.y==mine.y+1)
            if (board[final.x][final.y]!=NULL&&board[final.x][final.y]->colour==false)
                return true;
        if(final.x==mine.x+1&&final.y==mine.y-1)
            if (board[final.x][final.y]!=NULL&&board[final.x][final.y]->colour==false)
                return true;
        if(final.x==mine.x+2&&final.y==mine.y)
            if (mine.x==3&&board[final.x][final.y]==NULL) {
                return true;
            }
        return false;
    }
        ...
        if(final.x==mine.x-1&&final.y==mine.y+1)
            if (board[final.x][final.y]!=NULL&&board[final.x][final.y]->colour==true)
                return true;
        if(final.x==mine.x-1&&final.y==mine.y-1)
            if (board[final.x][final.y]!=NULL&&board[final.x][final.y]->colour==true)
                return true;
        if(final.x==mine.x-2&&final.y==mine.y)
            if (mine.x==8&&board[final.x][final.y]==NULL) {
                return true;
            }
        return false;
    }
...
bool rook::valid_move(piece *board[12][8],position final){
    if (final.x==mine.x) {
        int i=-7;
        if (final.y>mine.y) {
            for (i=mine.y+1; board[final.x][i]==NULL&&i<final.y; i++) {
        ...
        if (i==final.y) {
            if (board[final.x][final.y]==NULL||board[final.x][final.y]->colour!=colour) {
                return true;
            }
        }
    }
        ...
        int i=-7;
        if (final.x>mine.x) {
            for (i=mine.x+1; board[i][final.y]==NULL&&i<final.x; i++) {
...
bool bishop::valid_move(piece *board[12][8],position final){
    if (final.x-mine.x==final.y-mine.y) {
        int i=-7;
        if (final.y>mine.y) {
            for (i=1; board[mine.x+i][mine.y+i]==NULL&&i<final.y-mine.y; i++) {
        ...
        int i=-7;
        if (final.x>mine.x) {
            for (i=1; board[mine.x+i][mine.y-i]==NULL&&i<final.x-mine.x; i++) {
        ...
        if (final.y>mine.y) {
            if (colour==1) {
                ...
                if (!piece::hasblackkingmoved&&!piece::hasblackrook2moved) {
                    if (board[9][5]==NULL&&board[9][6]==NULL) {
            ...
            if (colour==1) {
                if (!piece::haswhitekingmoved&&!piece::haswhiterook1moved) {
                    if (board[2][1]==NULL&&board[2][2]==NULL&&board[2][3]==NULL) {
                ...
                if (!piece::hasblackkingmoved&&!piece::hasblackrook1moved) {
                    if (board[9][1]==NULL&&board[9][2]==NULL&&board[9][3]==NULL) {
